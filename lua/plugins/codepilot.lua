-- Codepilot settings
-- Select your chosen one in the last line
...
local codeium =
{
    ...
    config = function()
        ...
        vim.keymap.set({ "i", "s" }, '<a-n>', function() return vim.fn['codeium#CycleCompletions'](1) end,
            { expr = true })
        vim.keymap.set({ "i", "s" }, '<a-p>', function() return vim.fn['codeium#CycleCompletions'](-1) end,
            { expr = true })
